...
`get_data()` 和 `PartitionPlan` 的自动限额探测先不带业务筛选条件。
`add_api_params()` / 参数配置文件中的参数仅在接口拒绝无筛选探测时作为备用参数使用，
不会强制给本来不需要日期的接口加日期。超时仍按原来的顺序回退：无界请求失败后依次尝试
50 万、20 万、10 万、5 万、2 万、1 万、5 千条；不会通过加日期来绕过超时。
