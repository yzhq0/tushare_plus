...
兼容 DataCube 个别接口偶发比请求量多返回一行的情况。顺序分页按服务端实际返回量推进
offset，并在超量行与上一页边界重复时先去重、再应用调用方总 `limit`，避免数据遗漏和重复。
并发分页及超过一行的异常超量响应仍按严格协议校验处理。

## 0.1.12：恢复必要参数传递和零值缓存语义

修正 0.1.11 先发空参请求的回退策略：必要参数由调用者准备，库不推断或补全。
`_detect_api_limits()`、`get_api_info()` 和 `PartitionPlan` 的自动探测从第一次请求起
就原样携带 `add_api_params()` / 参数配置文件中已配置的参数，不先试空参。
当前查询和首个分区的筛选条件仍不会被自动提升为接口级探测参数。

已核对 DataCube 原生接口文档：[daily_basic（doc_id=32）](https://datacube.foundersc.com/document/2?doc_id=32)
要求 `ts_code` / `trade_date` 二选一；[daily（doc_id=27）](https://datacube.foundersc.com/document/2?doc_id=27)
将这两个参数列为可选。调用者应据此配置，不应要求库忽略已经提供的参数。

**内存及 CSV 中的 `limit_per_request=0` 仍表示无限制。** 读取零值不会重新探测或改写缓存；
普通查询和分区执行都按无限制单请求处理，不自动添加分页 `limit` / `offset`。
缺失记录与零值不再混淆。缓存决定请求方式，不代替响应的完整性校验。

### 保留的探测优化

修复 0.1.10 对已取尽的小表反复探测的问题。非空成功结果都会缓存到当前对象和限额 CSV；
后续查询、分区计划和新对象复用分页大小，不再每换一个日期就重新探测。空响应和全部探测失败的
降级值不持久化。分区计划仍只在工作线程启动前解析一次分页大小，不额外探测访问频率。

超时仍按原来的顺序回退：无界请求失败后依次尝试
50 万、20 万、10 万、5 万、2 万、1 万、5 千条；每次重试保留同一组必要参数，仅调整 `limit`。
实际查询保留调用者的筛选条件。`get_api_info(..., probe_params=...)` 的显式探测始终使用
指定参数，既不读取也不覆盖接口缓存。

分页大小是**后续请求的目标大小，不是已证实的服务端硬上限**：

- 非空且 `has_more=False`：返回行数加 25% 增长余量，向上取整；若该次探测指定了 `limit`，
  不超过该成功请求的 `limit`。例如 5,482 行缓存为 6,853，增长到 5,500 行仍可一次获取。
- `has_more=True` 或缺失：复用实际返回行数，不添加未被取尽证据支持的余量。
- 本次探测已明确取尽，且 API、字段和业务参数与当前查询完全一致、无显式切片时，
  直接使用这份响应并执行 schema/主键校验，不重复下载。缓存只保留大小和频率，不保留数据快照。

顺序分页仍按实际返回行数推进 offset；严格并发分页若遇到服务端静默缩小非末页，继续报错，
不会按较大的预估步长跳行。缺少 `has_more` 时仍需空页证据，不能因为“短于目标大小”就宣称取尽。
批量任务仍可显式传入 `limit_per_request` 跳过探测。

**已有偏小缓存按接口更新。** 升级不会删除全部历史记录。若 `daily` 等接口此前被日期筛选
或无增长余量的探测写入了偏小的值，先由调用者修正该接口的探测参数，再执行一次：

```python
client.force_redetect_api_limits("daily")
```

也可以用 `client.clear_api_limits("daily")` 清除该接口记录，让下次调用自动探测。
该操作只影响指定接口；若重新探测一次取尽，则保存带增长余量的请求页大小。
...
顺序模式会按实际返回行数推进 offset，因此可兼容服务端在某一页（通常是末页）
返回超过自动分页请求量的数据。若调用方指定总 `limit=N`，客户端会保留前 N 行，
并在分页报告中记录 `server_row_count` 以标示服务端超量返回；并发模式仍会拒绝
这种响应，因为预先规划的 offset 无法据此安全调整。
顺序分页会兼容恰好多返回一行的响应；若额外行是与上一页末行完全相同的页首行，
客户端会将其删除，并以 `duplicate_row_count` 记录。去重先于调用方总 `limit` 裁剪，
offset 始终按服务端实际返回量推进，避免重复行占用返回额度或因去重而跳过后续数据。
未超量的页面、页内以及非边界位置的相同行会原样保留。
