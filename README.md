...
`get_data()` 和 `PartitionPlan` 的自动探测统一使用 `add_api_params()` / 参数配置文件中的
API 预定义参数，不再继承当前查询或第一个分区的筛选条件。实际数据请求仍保留用户的筛选条件。
`get_api_info(..., probe_params=...)` 的显式探测独立于接口缓存，既不读取也不覆盖接口缓存。

小表一次取尽（`has_more=False`）只证明该次探测的行数，不能证明接口传输上限。
现在只有非空且 `has_more=True` 的探测结果才可缓存为已观察到的分页大小；这也不声称是服务端硬上限。
取尽、空响应、缺少 `has_more` 和探测失败的降级值只作为本次执行的保守步长，
不写入接口限额 CSV，下次调用重新探测。已经测得的访问频率在同一客户端内保留，避免重复频率探测。
分区计划仍只在启动工作线程前探测一次，不增加频率探测。

这避免小表当前行数在表增长后永久限制分页效率，也没有把“取尽”重新解释为“不限量”。
代价是无法确认上限的接口会在后续调用时再次探测；已独立验证分页大小的批量任务可显式传入
`limit_per_request` 跳过探测。并发分页仍会拒绝小于请求步长的非末页，防止跳行。

**已有缓存需要按接口修复。** 0.1.9 写入的正数无法可靠区分正确限额和窄查询行数，
升级不会自动删除全部历史记录。对受影响接口配置合适的必要探测参数后执行：
...
该操作只影响指定接口；若重新探测仍是一次取尽的小表，则不会重新写入未确认的上限。
